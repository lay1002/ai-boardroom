...
這 21 個 Gate 的實際中文名稱、`next_actor`、`recommended_execution_mode`、`risk_level`、狀態與下一步文字，屬於 Sprint-014 Architecture 未逐一列出的**實作填補**（見 `reviews/sprint-014/round-001/architecture.md` 第 0 節與 `claude_report.md`），由 Claude Code 依 `docs/development/consensus-workflow.md` 的流程語意合理判斷並記錄於 `scripts/review_bridge.sh` 的 `_gate_resolve_metadata` 函式中，供 Product Owner 之後審閱、確認或調整。
